...
def load_labels(path, dataset):
    labels = pd.read_csv("D:\\Pre-datasets\\datasets\\Baron_Human\\Labels.csv", index_col=None)
    labels.columns = ['V1']
...
def load_largesc(path, dirAdj, dataset, net):
    ...
    features = pd.read_csv("D:\Pre-datasets\datasets\Muraro\count\data_normalization.csv", index_col=0, header=0)
    features = findDuplicated(features)
    adj = pd.read_csv("D:\Pre-datasets\datasets\Muraro\count\Cosin_gene_0.6.csv", index_col=0)
...
def sparse_mx_to_torch_sparse_tensor(sparse_mx):
    ...
    # sparse_mx=scipy.sparse.spmatrix.tocoo().astype(np.float32)
    # sparse_mx = scipy.sparse.spmatrix.tocoo().astype(np.float32)
    adjedge_1 = torch.tensor(sparse_mx)
    sparse_mx = scipy.sparse.coo_matrix(adjedge_1)
    # rescombine_1 = np.vstack((my_matrix_1.row, my_matrix_1.col))
    # L = torch.tensor(rescombine_1).to(torch.long)
